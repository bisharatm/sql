...
select product_id, product_name, 
	...
	case 
	when product_name like '%pepper%' then 1 else 0 
	end as pepper_flag 
from product; 
...
select c.customer_id, c.customer_last_name, c.customer_first_name, sum(cp.quantity * cp.cost_to_customer_per_qty) as total_spending 
from customer as c 
